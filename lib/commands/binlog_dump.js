...
class QueryEvent {
  constructor(packet) {
    ...
    const statusVarsLength = packet.readInt16() & 0xffff;
    
    // Add bounds checking
    const remainingBeforeStatusVars = packet.end - packet.offset;
    const actualStatusVarsLength = Math.min(statusVarsLength, remainingBeforeStatusVars);
    
    const statusVars = packet.readBuffer(actualStatusVarsLength);
    this.schema = packet.readString(schemaLength, 'latin1');
    packet.readInt8(); // should be zero
    this.statusVars = parseStatusVars(statusVars);
    this.query = packet.readString(packet.end - packet.offset, 'latin1');
...
class XidEvent {
  constructor(packet) {
    this.binlogVersion = packet.readInt64();
